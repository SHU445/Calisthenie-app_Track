...
export default function ModifierSeancePage() {
  ...
  useEffect(() => {
    if (workout && exercises.length > 0) {
      ...
      // Grouper les sets par exercice pour reconstruire la structure avec séries individuelles
      ...
      const groupedSets = Object.entries(exerciseGroups).map(([exerciceId, sets]) => ({
        id: generateId(),
        exerciceId,
        series: sets.map(set => ({
          id: generateId(),
          ...
          tempsExecution: (set.duree && set.repetitions === 0) ? set.duree : undefined
        })),
        notes: sets[0].notes || '',
        // Déterminer le type de répétition basé sur les données existantes
        repetitionType: (sets[0].duree && sets[0].repetitions === 0) ? 'temps' : 'repetitions' as 'repetitions' | 'temps'
      }));

      setWorkoutSets(groupedSets);

      setIsLoading(false);
    } else if (workouts.length > 0 && !workout) {
      // Entraînement non trouvé
      router.push('/entrainements');
    }
  }, [workout, exercises.length, workouts.length, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ submit: 'Erreur lors de la modification de la séance' });
    } finally {
            ...
            <div className="max-w-4xl mx-auto">
              ...
              <div className="text-center mb-12">
                ...
                <p className="text-xl text-gray-300">
                  Modifiez votre séance d'entraînement
                </p>
              </div>
            </div>
          </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="sport-card p-8">
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-sport-accent mb-2">
                        Ressenti (1 = très facile, 5 = très difficile)
                      </label>
                          ...
                          {workoutSet.exerciceId && (
                            <div>
                              ...
                              <div className="mt-6">
                                ...
                                <textarea
                                  value={workoutSet.notes || ''}
                                  onChange={(e) => updateExerciseSet(workoutSet.id, 'notes', e.target.value)}
                                  className="martial-input w-full h-20 resize-none"
                                  placeholder="Observations, sensations, techniques..."
                                />
                              </div>
                            </div>
                          )}
                ...
                <div className="sport-card p-6">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                      <Link
                        href="/entrainements"
                        className="sport-btn-secondary text-center"
                      >
                        Annuler
                      </Link>
                      <button
                      ...
                      >
                        {isSubmitting ? 'Modification...' : 'Modifier la séance'}
                      </button>
                    </div>
                  </div>

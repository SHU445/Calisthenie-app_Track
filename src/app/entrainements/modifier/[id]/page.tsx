...
export default function ModifierSeancePage() {
  ...
  useEffect(() => {
    if (workout && exercises.length > 0) {
      ...
      // Grouper les sets par exercice pour reconstruire la structure avec nbSeries
      ...
      const groupedSets = Object.entries(exerciseGroups).map(([exerciceId, sets]) => ({
        ...
        poids: sets[0].poids,
        duree: sets[0].duree,
        tempsRepos: sets[0].tempsRepos,
        notes: sets[0].notes || '',
        // Déterminer le type de répétition et de charge basé sur les données existantes
        repetitionType: (sets[0].duree && sets[0].repetitions === 0) ? 'temps' : 'repetitions' as 'repetitions' | 'temps',
        tempsExecution: (sets[0].duree && sets[0].repetitions === 0) ? sets[0].duree : undefined,
        chargeType: 'corps' as 'poids' | 'corps'
      }));
  ...
  const getTotalEstimatedTime = () => {
    const exerciseTime = workoutSets.reduce((total, set) => {
      const exerciseTime = (set.duree || 30) * set.nbSeries;
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="text-sm text-gray-400">
                      <div className="flex items-center gap-4">
                        <span>{workoutSets.length} exercice{workoutSets.length > 1 ? 's' : ''}</span>
